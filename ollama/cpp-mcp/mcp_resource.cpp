...
text_resource::text_resource(const std::string& uri, const std::string& name,
...
bool text_resource::is_modified() const { return modified_; }

std::string text_resource::get_uri() const { return uri_; }
...
std::string text_resource::get_text() const { return text_; }
...
json binary_resource::read() const {
  ...
  if (!data_.empty()) {
    base64_data = macaron::Base64::Encode(
        std::string{reinterpret_cast<const char*>(data_.data()), data_.size()});
  }
  return {{"uri", uri_}, {"mimeType", mime_type_}, {"blob", base64_data}};
}

bool binary_resource::is_modified() const { return modified_; }

std::string binary_resource::get_uri() const { return uri_; }
...
const std::vector<uint8_t>& binary_resource::get_data() const { return data_; }
...
bool file_resource::is_modified() const {
  ...
  time_t current_modified =
      fs::last_write_time(file_path_).time_since_epoch().count();
...
std::shared_ptr<resource> resource_manager::get_resource(
...
int resource_manager::subscribe(
    const std::string& uri, std::function<void(const std::string&)> callback) {
  if (!callback) {
    throw mcp_exception(error_code::invalid_params,
                        "Cannot subscribe with null callback");
  }
