 ...
 * Protocol. Follows the 2024-11-05 protocol specification.
 */
...
class sse_client : public client {
 public:
   ...
   */
  sse_client(const std::string& host, int port = 8080,
   ...
   */
  sse_client(const std::string& base_url,
             const std::string& sse_endpoint = "/sse");
  ...
  bool initialize(const std::string& client_name,
                  const std::string& client_version) override;
  ...
  json call_tool(const std::string& tool_name,
                 const json& arguments = json::object()) override;
 ...
 private:
  ...
  void set_header_no_lock(const std::string& key, const std::string& value);
