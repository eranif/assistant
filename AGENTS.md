...
│   └── retry_policy.*      RetryPolicy interface + ExponentialBackoffRetryPolicy (429/529/5xx backoff)
...
tests/                      11 GoogleTest binaries (gated by ASSISTANTLIB_BUILD_TESTS or ENABLE_TESTS)
...
Each request goes through `ClientBase::SendWithRetry`, which consults the active `RetryPolicy` (default: `ExponentialBackoffRetryPolicy`). Retries are reported to callbacks as `Reason::kRetry`; the backoff sleep is interruptible via `Interrupt()`. Install a custom policy with `SetRetryPolicy(...)`, or `nullptr` to disable retries.

### Temp history (`ChatOptions::kNoHistory`) and usage tracking

`kNoHistory` requests swap `History` to a temporary store (`SwapToTempHistory`/`SwapToMainHistory`). Token accounting lives outside `History`: `SetLastRequestUsage` skips updating the last-request usage while the temp history is active, but aggregated usage (billing) always counts every request. `GetTokenUsageStats()` and `IsNearContextLimit()` therefore describe the **main** conversation (last main-history request); `GetAggregatedTokenUsageStats()` is cumulative.
...
- `.clang-format` — Google base, IndentWidth 2, ColumnLimit 80. There is no CI step that enforces this, but you **must** run `clang-format -i` on every C++ file you modify (only the files you touched) before committing.
...
- **Formatting:** always run `clang-format -i` on every C++ file you modify before committing.
- **Commits:** always sign off (`git commit -s`).
- **Commits:** never add a `Co-Authored-By` line to commit messages.
